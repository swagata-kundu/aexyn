...
import { SUPPLIER_INVITES, GET_INVITE_INFO } from '../../db/sp';
import { mfs_list } from './schema';
...
const fetchInviteInformations = (invite, qry, done) => {
  qry.query({
    ...
    ...invite, sentTo: r[0], reviewers: [], isCompanyLinked: true,
  })));
};
...
const getSupplierInvites = db => (req, res, next) => {
  ...
  Async.auto({
    validate: cb => Joi.validate(req.params, mfs_list, cb),
